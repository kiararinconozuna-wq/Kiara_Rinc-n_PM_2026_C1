...
*IX = 15;       /* Z[2], se modifica, aahora vale 15. */
printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);
...
++*IX;        /* Z[2] Se modifica, se incrementa en !> Z[2] ahora vale 11. */
*IX += 1;    /* Z[2] Se vuelve a modificar, ahora vale 12. */
printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);


 X = *(IX + 1);     /* X Se modifica> el apuntador IX accede temporalmente a Z[3], por lo tanto X toma este valor (8).
 observa quue IX no se resigna. */
 Y = *IX;          /* Y se modifica, toma el valor de Z[2} (12). */
 printf("\nX = %d \t Y = %d \t Z[0] = %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]); 

IX = IX + 1;    /* IX se modifica. observa la forma de mover el apuntador. 
ahora IX apunta al cuanto elemento de Z Z[3]. */
Y = *IX;        /* Y  se modifica, ahora vale Z[3] (8)*/

printf("\nX = %d \t Y = %d \t Z[0]= %d \t Z[1] = %d \t Z[2] = %d \t Z[3] = %d \t Z[4] = %d", X, Y, Z[0], Z[1], Z[2], Z[3], Z[4]);

IX = IX +4;            /* IX se modifica. Observa que el apuntador se mueve 4 posiciones  y caen en una direccion afuera del arreglo.
esto ocacionara un error. */
